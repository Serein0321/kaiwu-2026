...
| 1 | 敌方英雄 |
| 2 | 我方英雄 |
| 3~6 | 敌方4个小兵, 按runtime_id升序对齐官方target顺序 |
| 7 | 敌方防御塔 |
| 8 | 野怪/中立单位 |
...
4. 今年调试重点是鲁班七号扫射/照明buff与狄仁杰迅捷层数、二技能净化、三技能控制/破防状态
...
class DebugAgent:
    ...
    def act(self, info: Info):
        ...
        if self.camp == 1:    # 红方动作
            ...
            #     ### 鲁班七号/狄仁杰按1,2,3技能顺序调试buff、mark和子弹 ###
            #     # flag = False
            ...
            #             #         # flag = self.use_skill(0)    # 一技能
            #             #         flag = self.use_skill(2, 'hero')    # 三技能
            #             #     if flag:
            #             #         self.use_skill_count += 1
    ...
    def normal_attack(self, target=None):
        """target选择为
        'hero': 敌方英雄
        'soldier[0,1,2,3]: 按runtime_id升序排列的四个敌方小兵
        'organ': 最近敌方防御塔
        """
