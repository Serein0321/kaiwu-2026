    ...
    def learn(self, list_sample_data):
        ...
        for sample_data in list_sample_data:
            if isinstance(sample_data, np.ndarray):
                ...
                sample = getattr(sample_data, "sample", None)
                if sample is None:
                    sample = getattr(sample_data, "npdata", sample_data)
            sample_tensors.append(torch.as_tensor(sample))
        _input_datas = torch.stack(sample_tensors, dim=0)
