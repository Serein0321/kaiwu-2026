...
class EnvConfManager:
    ...
    def extract_hero_ids_from_usr_conf(usr_conf):
        ...
        def get_camp_hero_ids(camp_key):
            return [camp_info["hero_id"] for camp_info in lineups.get(camp_key, [])]

        return get_camp_hero_ids("blue_camp"), get_camp_hero_ids("red_camp")

    @staticmethod
    def inject_select_skills(usr_conf, camp_key, select_skills):
        lineups = usr_conf.get("lineups", {})
        for camp_info in lineups.get(camp_key, []):
            hero_id = camp_info.get("hero_id")
            if hero_id in select_skills:
                camp_info["select_skill"] = select_skills[hero_id]
                camp_info["summoner_skill_id"] = select_skills[hero_id]
    ...
    def update_config(self, lineup=None):
        ...
        if is_eval:
            eval_opponent_types = self.usr_conf["episode"].get("eval_opponent_types") or [
                self.usr_conf["episode"].get("eval_opponent_type", "common_ai")
            ]
            if os.environ.get("is_train_test", "False").lower() == "true":
                eval_opponent_types = [self.usr_conf["episode"].get("eval_opponent_type", "common_ai")]
            eval_opponent_idx = random.randint(0, len(eval_opponent_types) - 1)
            self.usr_conf['episode']['eval_opponent_type'] = eval_opponent_types[eval_opponent_idx]
