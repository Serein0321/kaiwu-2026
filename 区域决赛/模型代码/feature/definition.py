...
SampleData = create_cls("SampleData", sample=None, npdata=None)
...
def build_frame(agent, observation):
    ...
    frame_no = frame_state.get("frameNo", frame_state.get("frame_no", 0))
    for hero in hero_list:
        actor_state = hero.get("actor_state", hero)
        hero_camp = actor_state["camp"]
        hero_hp = actor_state["hp"]
        if hero_camp == agent.hero_camp or str(hero_camp).endswith(str(int(agent.hero_camp) + 1)):
    ...
    sub_action = sub_action_mask.get(str(action[0]), None) if isinstance(sub_action_mask, dict) else None
...
#作用是收集每个智能体的帧数据，并进行处理以生成训练样本。它计算累积奖励和优势函数（GAE），并根据LSTM时间步长将样本格式化为适合训练的数据结构。
...
def SampleData2NumpyData(g_data):
    if isinstance(g_data, np.ndarray):
        return g_data
    sample = getattr(g_data, "sample", None)
    if sample is not None:
        return sample
    npdata = getattr(g_data, "npdata", None)
    if npdata is not None:
        return npdata
    return np.asarray(g_data, dtype=np.float32)


def NumpyData2SampleData(s_data):
    return SampleData(sample=s_data, npdata=s_data)
